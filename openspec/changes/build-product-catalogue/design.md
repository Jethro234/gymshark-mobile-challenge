...
- Bullet rendering is proved by running the detail screen on a device or emulator **before**
  the rest of the screen is built out. This is a manual check, not an automated one — there is
  no snapshot layer to pin it (§8) and `AnnotatedString.fromHtml` cannot run in a JVM unit
  test, so there is no test layer in this project capable of exercising it at all.
- If bullets do not render, that is a version-floor defect to raise. It is explicitly **not**
  to be worked around by emitting `•` characters as body text — that would look right in a
...
from tokens, an aspect ratio reserved from payload `width`/`height` before load, and a
...
**No snapshot layer.** Paparazzi was planned as the middle tier — JVM-hosted, no emulator — but
its Gradle plugin is incompatible with AGP 9, which Hilt's Gradle plugin (2.60.1) hard-requires.
Both cannot be satisfied at once, and Hilt is used throughout the app while Paparazzi is one
test layer, so `docs/SCOPE.md` §6's pre-authorised fallback applies: the layer is dropped rather
than AGP held back. Concretely this means:

- The bullet-rendering proof and the RTL mirroring proof (both originally goldens) are now
  manual, on-device checks performed before their respective screens are considered complete
  (§2, and `design-system-and-accessibility`'s RTL requirement).
- The image error state's visibility (`product-imagery`) is unaffected — it was always proved
  by a fixture with a dead URL, visible on first launch, not by a golden.
- Everything else DESIGN.md §8 assigned to the Paparazzi matrix — light/dark variants, label
  badge treatments, font-scale reflow — has no remaining automated coverage in this project.
  The unit suite and the two instrumented tests carry the grade instead.
...
| Risk | Mitigation |
|---|---|
| **`ui-text` 1.9's bullet support does not behave as documented** → the graded requirement fails late | Verified in task 1 when the BOM is pinned, and confirmed by manual on-device check before the detail screen is built. Raise it; do not fake bullets with text characters |
...
contingency in `docs/SCOPE.md` §6 fired during implementation, though the actual cause was AGP
version incompatibility with Hilt rather than JDK friction. See §8 above and commit `be868e3`
for the full chain of evidence. The unit suite and the two instrumented tests carry the grade in
its place.
...
- `README.md`'s testing table lists Paparazzi and describes snapshot coverage as "every
  `UiState`, every label treatment". The layer no longer exists at all — see §8 — so this row
  needs removing, not narrowing to a smaller tier. `docs/DESIGN.md` §8's Paparazzi state matrix
  and Tier 1/Tier 2 split are stale for the same reason.
