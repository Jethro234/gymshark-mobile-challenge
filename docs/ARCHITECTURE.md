...
The endpoint returns an **Algolia search response** (`hits`, `nbHits`, `page`,
`hitsPerPage`) containing **10 products**. Reading it carefully changes several design
decisions:

1. **`price` is `1000` on nine of ten products, `50` on the tenth.** Interpreted as
   *minor units* (pence) — see §7.
2. **`labels` is `null` on nine products; one has `["going-fast"]`.** The brief's headline
   feature has exactly one observable value in the fixture. See §8.
...
7. **Ten hits are five products in ten colourways.** `Speed Leggings` ×3,
   `Pulse Mesh Leggings` ×3, `Adapt Camo Seamless Leggings` ×2,
   `Vital Seamless 2.0 Leggings` ×2, `Adapt Animal Seamless Leggings` ×1 — and `handle`
   encodes it (`gymshark-speed-leggings-navy-ss22`). The list ships **flat**, as the search
...
**Assumption: `price` and `compareAtPrice` are in minor units (pence).**
`1000` → £10.00, `50` → £0.50.

Justification: Shopify and Algolia conventionally store minor units; and £1,000 leggings
sitting alongside a £50 accessory at otherwise identical prices is implausible for the
brand. The assumption is stated in the README — a reasoned, documented assumption is worth
more than a silent correct guess.

```kotlin
@JvmInline
value class Money(val minorUnits: Long) {
    companion object { const val MINOR_UNITS_PER_MAJOR = 100L }
}
```
...
- `MINOR_UNITS_PER_MAJOR` is a single named constant, so if the assumption is wrong it is a
  one-line reversal, and both interpretations are covered by unit tests.
- `compareAtPrice` non-null and greater than `price` drives the strikethrough and the
  ...
  (recomputing risks disagreeing with the merchandiser's own figure).
...
sealed interface Label {
    val raw: String
    data object GoingFast   : Label { override val raw = "going-fast" }
    data object NewIn       : Label { override val raw = "new-in" }
    data object BackInStock : Label { override val raw = "back-in-stock" }
    data object SoldOut     : Label { override val raw = "sold-out" }
    data class Unknown(override val raw: String) : Label
}
```

- **`Unknown(raw)` is the point.** A merchandiser adding a label in Shopify next week must
  not crash the app or produce a blank badge. Unknown labels render as a neutral chip with
  the raw value title-cased, so new data degrades gracefully rather than disappearing.
- Parsing is null-safe, case-insensitive and trims — `labels` is `null` far more often than
  it is populated.
- Known labels get distinct badge styling; `GoingFast` is the only one exercised by the
  live payload, so the others are covered by fixtures and snapshot tests.

**Rejected:** modelling only `going-fast` (honest to the fixture, but silently drops data
the brief says to surface); raw strings only (never drops data, but no per-label styling
and the domain layer learns nothing).
...
| `Money` | Minor→major conversion, locale formatting, zero, discount comparison |
| `Label` | Known parsing, case-insensitivity, `Unknown` fallback, null and empty arrays |
...
> endpoint returns a complete 10-item response with no paging mechanism behind it, so
...
`Room` was likewise rejected: a schema, DAOs and migrations for ten products that never
...
1. `price` / `compareAtPrice` are in minor units (§7).
2. Currency is GBP; formatting is locale-aware and would follow a real currency code if the
   API supplied one.
3. Label vocabulary beyond `going-fast` is inferred; `Unknown` handles anything else.
4. `discountPercentage` is displayed as supplied, never recalculated.
...
| **Colourway grouping** | One card per title with colour swatches; the ten hits are five products | Would mean inventing a product model the search API does not express |
